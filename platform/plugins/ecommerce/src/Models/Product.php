...
use Botble\MixAndMatch\Traits\HasMixAndMatch;
...
class Product extends BaseModel
{
    ...
    {
        // For variation products, check if they have their own flash sale entries
        // If not, fall back to the parent product's flash sales
        if ($this->is_variation) {
            // First, try to get flash sales directly associated with this variation
            $directFlashSales = $this->belongsToMany(FlashSale::class, 'ec_flash_sale_products', 'product_id', 'flash_sale_id')
                ->withPivot(['price', 'quantity', 'sold']);
            
            // If this variation has its own flash sales, return them
            if ($directFlashSales->exists()) {
                return $directFlashSales;
            }
            
            // Otherwise, fall back to the parent product's flash sales
            return $this->original_product
                ->belongsToMany(FlashSale::class, 'ec_flash_sale_products', 'product_id', 'flash_sale_id')
                ->withPivot(['price', 'quantity', 'sold']);
        }
        
        // For regular products, return their flash sales directly
