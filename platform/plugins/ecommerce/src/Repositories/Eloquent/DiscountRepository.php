...
class DiscountRepository extends RepositoriesAbstract implements DiscountInterface
{
    public function getAvailablePromotions(array $with = [], bool $forProductSingle = false)
    {
        ...
        $promotions = $this->applyBeforeExecuteQuery($data)->get();

        // Filter the promotions to only include those active for the current day
        return $promotions->filter(function ($promotion) {
            return $promotion->isActiveForCurrentDay();
        });
    }
