...
trait ProductActionsTrait
{
    ...
    public function getListProductForSearch(ProductListRequest $request, BaseHttpResponse $response): BaseHttpResponse
    {
        ...
        $includeVariation = $request->input('include_variation', 1); // Default to include variations
        
        // Build a simple query that works for both parent and variation products
        $query = Product::query()
        ...
        // Only filter by is_variation if we don't want to include variations
        if (!$includeVariation) {
            $query->where('is_variation', 0);
        }
        
        // Load with relationships - use variationInfo and variationProductAttributes
        $query->with(['variationInfo']);
        
        // Get the products
        $availableProducts = $query->simplePaginate(10);
        
        // Process each product to add attribute information for variations
        foreach ($availableProducts as $product) {
            if ($product->is_variation) {
                // Get variation attributes
                $attributeInfo = $this->getVariationAttributeInfo($product);
                if (!empty($attributeInfo)) {
                    // Append attributes to name if not already there
                    if (strpos($product->name, '(' . $attributeInfo . ')') === false) {
                        $product->display_name = $product->name . ' (' . $attributeInfo . ')';
                    } else {
     ...
     * @return string
     */
    protected function getVariationAttributeInfo($product): string
    {
        if (!$product->is_variation) {
            return '';
        }
        ...
        try {
            // First, try to get variation attributes using variationInfo relationship
            if ($product->variationInfo) {
                ...
                if ($variationItems->isNotEmpty()) {
                    ...
                    foreach ($variationItems as $item) {
                        $result[] = $item->attribute_set . ': ' . $item->attribute_value;
                    }
                    return implode(', ', $result);
                }
            }
            
            // If no attributes found via variationInfo, try direct query using the product ID
            // This is a fallback for products that might not have proper relationships set up
            ...
            // Try to extract from product name if it contains parentheses
