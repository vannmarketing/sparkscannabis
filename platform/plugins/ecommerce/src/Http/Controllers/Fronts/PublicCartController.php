...
class PublicCartController extends BaseController
{
    ...
    public function store(CartRequest $request)
    {
        ...
        $maxQuantity = $product->maximum_order_quantity;

        $requestQuantity = $request->integer('qty', 1);

        // Check if the product is already in the cart
        $existingCartItems = Cart::instance('cart')->content()->filter(function ($item) use ($product) {
            return $item->id == $product->id;
        });
        
        $currentCartQuantity = $existingCartItems->sum('qty');
        $totalQuantity = $currentCartQuantity + $requestQuantity;
        
        // Check if adding this quantity would exceed the maximum order quantity
        if ($maxQuantity && $totalQuantity > $maxQuantity) {
            return $response
                ->setError()
                ->setMessage(__('Sorry, you can only order a maximum of :quantity units of :product per order. You already have :current_quantity in your cart.', [
                    'quantity' => $maxQuantity, 
                    'product' => $product->name,
                    'current_quantity' => $currentCartQuantity
                ]));
        }
