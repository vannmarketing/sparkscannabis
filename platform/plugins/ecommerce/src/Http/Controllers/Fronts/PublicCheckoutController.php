...
class PublicCheckoutController extends BaseController
{
    ...
    public function getCheckout(
        ...
        // Filter the fetched discounts to only include those active for the current day
        $activeDiscounts = $discounts->filter(function ($discount) {
            return $discount->isActiveForCurrentDay();
        });

        // Filter out coupons that the customer has already used
        if (auth('customer')->check()) {
            $customerId = auth('customer')->id();
            $activeDiscounts = $activeDiscounts->filter(function ($discount) use ($customerId) {
                // If the coupon is for one-time use per customer and the customer has already used it, filter it out
                if ($discount->target == DiscountTargetEnum::ONCE_PER_CUSTOMER) {
                    return !$discount->usedByCustomers()->where('customer_id', $customerId)->exists();
                }
        ...
        $data = [...$data, 'discounts' => $activeDiscounts, 'rawTotal' => $rawTotal, 'orderAmount' => $orderAmount];
    ...
    public function postCheckout(
        ...
        // Check minimum order amount based on the "Apply After Discount" setting
        $minimumOrderAmount = EcommerceHelper::getMinimumOrderAmount();
        if ($minimumOrderAmount > 0) {
            $sessionData = OrderHelper::getOrderSessionData($token);
            $currentAmount = Cart::instance('cart')->rawSubTotal();
            
            // If "Apply After Discount" is enabled, use the total after discounts
            if (EcommerceHelper::isMinimumOrderAmountAppliedAfterDiscount()) {
                $currentAmount = $currentAmount
                    - Arr::get($sessionData, 'promotion_discount_amount', 0)
                    - Arr::get($sessionData, 'coupon_discount_amount', 0);
            }
            
            if ($minimumOrderAmount > $currentAmount) {
                return $this
                    ->httpResponse()
                    ->setError()
                    ->setMessage(
                        __('Minimum order amount is :amount, you need to buy more :more to place an order!', [
                            ...
                            'more' => format_price($minimumOrderAmount - $currentAmount),
                        ])
                    );
            }
        }
