        ...
        if (FlashSale::isEnabled()) {
            foreach ($orders as $order) {
                foreach ($order->products as $orderProduct) {
                    $purchasedProduct = $orderProduct->product;
                    
                    // First, check if this exact product (whether variation or regular) has a direct flash sale entry
                    $directFlashSale = DB::table('ec_flash_sale_products')
                        ->join('ec_flash_sales', 'ec_flash_sales.id', '=', 'ec_flash_sale_products.flash_sale_id')
                        ->where('ec_flash_sale_products.product_id', $purchasedProduct->id)
                        ->where('ec_flash_sales.status', 'published')
                        ->where('ec_flash_sales.end_date', '>=', now())
                        ->where('ec_flash_sale_products.quantity', '>', DB::raw('ec_flash_sale_products.sold'))
                        ->select(
                            'ec_flash_sale_products.*',
                            'ec_flash_sales.id as flash_sale_id'
                        )
                        ->first();
                    
                    if ($directFlashSale) {
                        // Update the flash sale counter for this exact product
                        DB::table('ec_flash_sale_products')
                            ->where('flash_sale_id', $directFlashSale->flash_sale_id)
                            ->where('product_id', $purchasedProduct->id)
                            ->update([
                                'sold' => DB::raw('sold + ' . $orderProduct->qty),
                            ]);
                        continue;
                    }
                    
                    // If it's a variation but doesn't have its own flash sale entry, check the parent product
                    if ($purchasedProduct->is_variation) {
                        $parentProduct = $purchasedProduct->original_product;
                        if (!$parentProduct) {
                            continue;
                        }
                        
                        $parentFlashSale = DB::table('ec_flash_sale_products')
                            ->join('ec_flash_sales', 'ec_flash_sales.id', '=', 'ec_flash_sale_products.flash_sale_id')
                            ->where('ec_flash_sale_products.product_id', $parentProduct->id)
                            ->where('ec_flash_sales.status', 'published')
                            ->where('ec_flash_sales.end_date', '>=', now())
                            ->where('ec_flash_sale_products.quantity', '>', DB::raw('ec_flash_sale_products.sold'))
                            ->select(
                        ...
                        if ($parentFlashSale) {
                            // Update the flash sale counter for the parent product
                            DB::table('ec_flash_sale_products')
                                ->where('flash_sale_id', $parentFlashSale->flash_sale_id)
