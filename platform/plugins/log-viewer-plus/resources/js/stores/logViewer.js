...
export const useLogViewerStore = defineStore({
    ...
    getters: {
        ...
        stickTopPosition() {
            return (index) => {
                ...
                if (aboveFold < 0) {
                    return Math.max(this.headerHeight - this.tableRowHeight, this.headerHeight + aboveFold) + 'px'
                }
    ...
    actions: {
        ...
        loadLogs: debounce(function ({ silently = false } = {}) {
            ...
            axios
                .get(`${LogViewer.basePath}/api/logs`, { params, signal: this.abortController.signal })
                .then(({ data }) => {
                    if (params.host) {
