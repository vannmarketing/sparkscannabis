...
export const useFileStore = defineStore({
    ...
    getters: {
        ...
        foldersOpen(state) {
            return state.openFolderIdentifiers.map((identifier) =>
                state.folders.find((folder) => folder.identifier === identifier)
            )
        },
    ...
    actions: {
        ...
        selectFile(logFileIdentifier) {
            if (this.selectedFileIdentifier === logFileIdentifier) {
                return
            }
        ...
        openFolderForActiveFile() {
            if (this.selectedFile) {
                const folder = this.folders.find((folder) =>
                    folder.files.some((file) => file.identifier === this.selectedFile.identifier)
                )
        ...
        loadFolders() {
            ...
            return axios
                .get(`${LogViewer.basePath}/api/folders`, {
                    params: {
                ...
                .then(({ data }) => {
                    ...
                    if (!this.selectedFileIdentifier && this.files.length > 0) {
                        this.selectFile(this.files[0].identifier)
                    }
        ...
        clearCacheForFile(file) {
            ...
            return axios
                .post(
                    `${LogViewer.basePath}/api/files/${file.identifier}/clear-cache`,
                    {},
                    {
        ...
        deleteFile(file) {
            return axios
                .delete(`${LogViewer.basePath}/api/files/${file.identifier}`, {
                    params: { host: this.hostQueryParam },
                })
        ...
        clearCacheForFolder(folder) {
            ...
            return axios
                .post(
                    `${LogViewer.basePath}/api/folders/${folder.identifier}/clear-cache`,
                    {},
                    {
                        params: { host: this.hostQueryParam },
                    }
        ...
        deleteFolder(folder) {
            ...
            return axios
                .delete(`${LogViewer.basePath}/api/folders/${folder.identifier}`, {
                    params: { host: this.hostQueryParam },
                })
                .then(() => this.loadFolders())
                .catch((error) => console.error(error))
        ...
        deleteSelectedFiles() {
            return axios.post(
                `${LogViewer.basePath}/api/delete-multiple-files`,
                {
        ...
        clearCacheForAllFiles() {
            ...
            axios
                .post(
                    `${LogViewer.basePath}/api/clear-cache-all`,
                    {},
                    {
                        params: { host: this.hostQueryParam },
                    }
