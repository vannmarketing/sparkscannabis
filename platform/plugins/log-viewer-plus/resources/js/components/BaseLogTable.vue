<template>
    <table class="max-w-full min-w-full border-separate table-fixed" style="border-spacing: 0">
        ...
        <template v-if="logViewerStore.logs && logViewerStore.logs.length > 0">
            <tbody
                v-for="(log, index) in logViewerStore.logs"
                :key="index"
            ...
            >
                <tr
                    @click="logViewerStore.toggle(index)"
                    :class="[
                        'log-item group',
                        log.level_class,
                        logViewerStore.isOpen(index) ? 'active' : '',
                        logViewerStore.shouldBeSticky(index) ? 'sticky z-2' : '',
                    ]"
                    :style="{ top: logViewerStore.stackTops[index] || 0 }"
                >
                    <td class="hidden log-level lg:table-cell">
                        <div class="flex items-center lg:pl-2">
                            <button
                                :aria-expanded="logViewerStore.isOpen(index)"
                                @keydown="handleLogToggleKeyboardNavigation"
                                class="hidden w-5 h-5 rounded-md opacity-75 log-level-icon lg:block group focus:opacity-100 focus:outline-none focus:ring-2 focus:ring-brand-500"
                            >
                                ...
                                <span class="hidden w-full h-full group-hover:inline-block group-focus:inline-block">
                                    <ChevronRightIcon
                                        :class="[
                                            logViewerStore.isOpen(index) ? 'rotate-90' : '',
                    ...
                    <template v-for="(column, colIndex) in logViewerStore.columns">
                        ...
                        <td
                            ...
                            class="text-gray-900 whitespace-nowrap dark:text-gray-200"
                        >
                            <span
                                class="hidden lg:inline"
                                v-html="highlightSearchResult(log.datetime, searchStore.query)"
                            ></span>
                        ...
                        >
                            <span
                                v-html="highlightSearchResult(getDataAtPath(log, column.data_path), searchStore.query)"
                            ></span>
                        </td>
                    </template>

                    <td
                        class="hidden text-xs text-gray-500 whitespace-nowrap dark:text-gray-300 dark:opacity-90 lg:table-cell"
                    >
                ...
                <tr v-show="logViewerStore.isOpen(index)">
                    <td colspan="6">
                        <div class="flex justify-between px-2 pt-2 pb-1 text-xs lg:hidden">
                        ...
                        <tab-container v-if="logViewerStore.isOpen(index)" :tabs="getTabsForLog(log)">
                            ...
                            <tab-content tab-value="raw">
                                <pre
                                    class="log-stack"
                                    v-html="highlightSearchResult(log.full_text, searchStore.query)"
                                ></pre>
                                <template v-if="hasContext(log)">
                                    <p
                                        class="pt-2 mx-2 text-xs font-semibold text-gray-700 border-t lg:mx-8 dark:text-gray-400 lg:text-sm"
                                    >
                                        Context:
                                    </p>
                                    <pre
                                        ...
                                        v-html="
                                            highlightSearchResult(
                                                prepareContextForOutput(log.context),
                                                searchStore.query
                                            )
                                        "
                                    ></pre>
                                ...
                                <div
                                    v-if="log.extra && log.extra.log_text_incomplete"
                                    class="px-8 py-4 italic text-gray-500"
                                >
                                    The contents of this log have been cut short to the first
                                    {{ LogViewer.max_log_size_formatted }}. The full size of this log entry is
                                    <strong>{{ log.extra.log_size_formatted }}</strong>
                                </div>
                            </tab-content>
                        </tab-container>
                    </td>
        ...
        <tbody v-else class="log-group">
            <tr>
                <td colspan="6">
                    <div class="p-12 text-gray-600 bg-white dark:bg-gray-800 dark:text-gray-200">
                        <div class="font-semibold text-center">No results</div>
                        <div class="mt-6 text-center">
                            <button
                                v-if="searchStore.query?.length > 0"
                                class="px-3 py-2 text-gray-800 border rounded-md dark:border-gray-700 dark:text-gray-200 hover:border-brand-600 dark:hover:border-brand-700"
                                @click="clearQuery"
                            >
                                Clear search query
                            </button>
                            <button
                                v-if="searchStore.query?.length > 0 && fileStore.selectedFile"
                                class="px-3 py-2 ml-3 text-gray-800 border rounded-md dark:border-gray-700 dark:text-gray-200 hover:border-brand-600 dark:hover:border-brand-700"
                            ...
                            <button
                                ...
                                class="px-3 py-2 ml-3 text-gray-800 border rounded-md dark:border-gray-700 dark:text-gray-200 hover:border-brand-600 dark:hover:border-brand-700"
                                @click="severityStore.selectAllLevels"
                            >
...
const getExtraTabsForLog = (log) => {
    return [log.extra && log.extra.mail_preview ? { name: 'Mail preview', value: 'mail_preview' } : null].filter(
        Boolean
    )
}
const getTabsForLog = (log) => {
    return [...getExtraTabsForLog(log), { name: 'Raw', value: 'raw' }].filter(Boolean)
}
